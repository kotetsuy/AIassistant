...
- **実施結果(2026-08-13): 完了**。13音源(短文9 + 長文4)× 3構成 × 20回
  ...
  - **エンドツーエンド**(発話終了→最初の音声、3.5秒発話・各30回): whisperX **895ms** → NeMo 一括 **735ms** / ストリーミング **614ms**(中央値)。効いているのは分布で、**1秒超が 11/30 → 0/30**。ただしセッションごとに中央値が 895〜1074ms とぶれるため「1秒を超える割合」で見る方が頑健
